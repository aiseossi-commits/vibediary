...
무료 유저의 월별 AI 호출(등대 질문 + AI 인사이트 생성) 횟수를 추적하고 한도(10회)를 적용한다. 유료 유저는 무제한.
...
앱은 무료 유저의 등대 질문과 AI 인사이트 생성 호출을 월별로 추적해야 한다. 카운터는 `app_settings` DB에 `ai_usage_count`(정수)와 `ai_usage_month`(YYYY-MM 문자열) 키로 저장한다.
...
- **THEN** "이번 달 AI N회 남음" 텍스트를 표시한다

#### Scenario: 소진 시 버튼 비활성화
